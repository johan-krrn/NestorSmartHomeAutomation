...
// ---------------------------------------------------------------------------
// Logging helpers (never leak the token)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// REST call to Home Assistant Supervisor API (using built-in http module)
// ---------------------------------------------------------------------------
...
async function validateToken() {
  ...
  try {
    ...
    if (result.status === 200) {
      log("INFO", "HA token is valid. API reachable.");
      return true;
    }
    log("ERROR", `HA returned status ${result.status} during token validation.`);
    return false;
  } catch (err) {
    log("ERROR", "Cannot reach HA API during token validation.", {
      error: err.message,
    });
    return false;
  }
...
function validateMessage(data) {
  ...
  if (typeof data.endpoint !== "string" || !data.endpoint.trim()) {
    return 'Missing or invalid "endpoint" field.';
  }
  // Prevent path traversal — endpoint must start with /api/
  if (!data.endpoint.startsWith("/api/")) {
    return 'Endpoint must start with "/api/".';
  }
  const method = data.method.toUpperCase();
  if (!ALLOWED_METHODS.includes(method)) {
    return `Method "${method}" is not allowed. Allowed: ${ALLOWED_METHODS.join(", ")}`;
  }
  if (["POST", "PUT", "PATCH"].includes(method) && data.payload == null) {
    return `Payload is required for ${method} requests.`;
  }
  return null;
}

// ---------------------------------------------------------------------------
// Retry wrapper for HA connectivity
// ---------------------------------------------------------------------------
async function callHAWithRetry(method, endpoint, payload, retries = 3, delayMs = 2000) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      log("ERROR", `HA request failed (attempt ${attempt}/${retries}).`, {
        error: err.message,
      });
...
async function main() {
  if (!HA_TOKEN) {
    log("ERROR", "HA_TOKEN is not set. Aborting.");
    process.exit(1);
  }
  ...
  if (!tokenValid) {
    log(
      "ERROR",
      "Could not validate HA token at startup. The addon will start anyway but requests may fail."
    );
  }
  ...
  wss.on("connection", (ws, req) => {
    ...
    ws.on("message", async (raw) => {
      ...
      if (validationError) {
        const errResp = { success: false, error: validationError };
        ws.send(JSON.stringify(errResp));
      ...
      // Forward to HA
      try {
        const result = await callHAWithRetry(
          data.method,
          data.endpoint,
          data.payload
        );
        ...
        ws.send(JSON.stringify(errResp));
        log("ERROR", "HA API call failed after retries.", {
          error: err.message,
        });
      }
    });
    ...
    ws.on("error", (err) => {
      log("ERROR", `WebSocket error for ${clientIp}`, { error: err.message });
    });
  });
